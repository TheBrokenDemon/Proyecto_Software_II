...
const submitResponses = async (req, res, next) => {
    try {
        const result = await (0, evaluation_service_1.submitEvaluationResponses)(req.user.id, req.params.slug, req.body.answers);
...
const myHistory = async (req, res) => {
    try {
        const history = await (0, evaluation_service_1.getMyHistory)(req.user.id);
