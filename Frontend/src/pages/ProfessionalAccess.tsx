...
export default function ProfessionalAccess() {
    ...
    const [formData, setFormData] = useState<FormData>({
        nombres: '', correo: '', codigoAutorizacion: '', especialidad: '',
    });
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        setTimeout(() => navigate('/professional-panel'), 1200);
    };

    return (
        <main className="professional-access-page">
            <section className="professional-access-card">
                <Link to="/login" className="professional-back">← Volver al login</Link>

                <div className="professional-header">
                    <div className="professional-icon">🧠</div>
                    <h1>Acceso profesional</h1>
                    <p>Completa tus datos para validar el acceso al panel de seguimiento psicológico estudiantil.</p>
                </div>
                ...
                <form className="professional-form" onSubmit={handleSubmit}>
                    <div className="form-group">
                        ...
                        <input type="text" name="nombres" value={formData.nombres}
                            onChange={handleChange} placeholder="Ejemplo: María López Ramírez" />
                    </div>
                    <div className="form-group">
                        <label>Correo institucional</label>
                        <input type="email" name="correo" value={formData.correo}
                            onChange={handleChange} placeholder="profesional@ulima.edu.pe" />
                    </div>
                    <div className="form-group">
                    ...
                    <div className="form-group">
                        ...
                        <input type="password" name="codigoAutorizacion" value={formData.codigoAutorizacion}
                            onChange={handleChange} placeholder="Ingresa tu código de acceso" />
                    </div>

                    {error && <p className="professional-error">{error}</p>}
                    {success && <p className="professional-success">{success}</p>}

                    <button type="submit" className="professional-submit">Validar acceso</button>
                </form>

                <div className="professional-help">
                    <p>Para la demostración del MVP, el código autorizado es: <strong>PSICO-ULIMA-2026</strong></p>
                </div>
            </section>
        </main>
    );
